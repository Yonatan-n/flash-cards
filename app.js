...
function loadStore() {
  try {
    ...
    alert(`Failed to load saved data.\n\n${e.message}\n\nStarting with a fresh session. Your progress may be lost.`);
  }
...
async function loadWords() {
  // Prefer a user-imported word list stored in localStorage
  ...
  ALL_WORDS = parseWordlistCSV(text);
}
...
function showHome() {
  ...
  render(`
    <div class="flex flex-col flex-1 px-6 pt-12 pb-8 gap-6">
      ...
      <div class="flex gap-3">
        <button id="btn-export-progress" class="flex-1 py-3 rounded-xl text-sm font-medium bg-indigo-900 active:bg-indigo-800 active:scale-95 transition-all">
          ${S.exportProgress}
        </button>
        <label class="flex-1 py-3 rounded-xl text-sm font-medium bg-indigo-900 active:bg-indigo-800 active:scale-95 transition-all text-center cursor-pointer">
          ${S.importProgress}
          <input type="file" accept=".json" class="hidden" id="import-progress-file" />
        </label>
      </div>
      <div class="flex gap-3">
        <button id="btn-export-wordlist" class="flex-1 py-3 rounded-xl text-sm font-medium bg-indigo-900 active:bg-indigo-800 active:scale-95 transition-all">
          ${S.exportWordlist}
        </button>
        <label class="flex-1 py-3 rounded-xl text-sm font-medium bg-indigo-900 active:bg-indigo-800 active:scale-95 transition-all text-center cursor-pointer">
          ${S.importWordlist}
          <input type="file" accept=".csv" class="hidden" id="import-wordlist-file" />
        </label>
      </div>
  ...
  document.getElementById('import-progress-file').onchange = importProgress;
  document.getElementById('btn-export-wordlist').onclick = exportWordlist;
  document.getElementById('import-wordlist-file').onchange = importWordlist;
}
...
let learnTransitioning = false;

function nextCard() {
  if (learnTransitioning || cardIndex >= learnWords.length - 1) return;
  learnTransitioning = true;
...
function prevCard() {
  if (learnTransitioning || cardIndex <= 0) return;
  learnTransitioning = true;
  app.style.transition = 'opacity 0.2s ease';
  app.style.opacity = '0';
  setTimeout(() => {
      ...
      <div class="flex flex-col gap-3 flex-1">
        ${testResults.map(r => `
          <div class="rounded-2xl px-5 py-4 ${r.correct ? 'bg-emerald-900/60 border border-emerald-700' : 'bg-red-900/60 border border-red-700'}">
            ...
            ${!r.correct ? `
              <div class="text-red-300 text-xs mt-0.5 flex flex-col">
                <span>${r.dir === 'de' ? 'You typed:' : 'כתבת:'}</span>
                <span class="${r.dir === 'de' ? 'he' : ''}">${r.input}</span>
              </div>
            ` : ''}
          </div>
        `).join('')}
...
function setImportMsg(text, isError = false) {
  const el = document.getElementById('import-msg');
  if (!el) return;
  el.textContent = text;
  el.className = `text-center text-sm h-4 ${isError ? 'text-red-400' : 'text-indigo-300'}`;
}

// ── Progress ──
function exportProgress() {
  const st = loadStore();
  downloadBlob(new Blob([JSON.stringify(st, null, 2)], { type: 'application/json' }), 'flashcards_progress.json');
}
...
function importProgress(e) {
  ...
  reader.onload = ev => {
    try {
      ...
      saveStore(data);
      setImportMsg(S.progressImportDone);
      setTimeout(() => showHome(), 800);
    } catch {
      setImportMsg(S.progressImportError, true);
    }
  };
...
function exportWordlist() {
  const rows = ALL_WORDS.map(w => `${w.german},${w.hebrew}`).join('\n');
  const csv = `german,hebrew\n${rows}`;
  downloadBlob(new Blob([csv], { type: 'text/csv' }), 'words_clean.csv');
}

function importWordlist(e) {
  const file = e.target.files[0];
  if (!file) return;
  const reader = new FileReader();
  reader.onload = ev => {
    try {
      const words = parseWordlistCSV(ev.target.result);
      if (words.length === 0) throw new Error('No valid rows found');
